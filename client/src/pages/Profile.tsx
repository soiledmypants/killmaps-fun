      ...
      <div className="panel p-6 mb-4">
        ...
        <p className="text-sm text-steel leading-relaxed">
          {player?.verified ? (
            ...
            <>Hold at least <span className="text-accent font-mono">{(config?.minTokens ?? 250000).toLocaleString()}</span> tokens in your payout wallet to become verified. {config && !config.verifyLive && "(Server verification is in mock mode.)"}</>
          )}
        </p>
      </div>
